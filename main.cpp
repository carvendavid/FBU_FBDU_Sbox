/*********************************************************************
* Filename:   main.cpp
* Authors:     David Carcaño Ventura (carvendavid@gmail.com), 
*  			   Lil Rodríguez Henríquez (lmrodriguez@inaoep.mx),
*			   Saúl Pomares Hernández (spomares@inaoep.mx)
* Data:       June 20, 2023
* Using:      To create a tool for analyzing S-box
* Details:    
*********************************************************************/

#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <iostream>
#include <sys/time.h>
#include <bitset>
#include "math.h"
#include <stdint.h>

//mis librerias
#include "fbct.h"
#include "fbdt.h"
#include "lat.h"
#include "ddt.h"
#include "string_uchar.h"

using namespace std;

int main(){


	  /***********************Sboxes************************/
/*Caja de SAND[7] junta FBU=8*/
	/*unsigned char SBOX[] = {
  	    0x00,0x11,0x22,0xb3,0x44,0x57,0x66,0xf5,
  	    0x88,0x99,0xae,0x3d,0xdc,0xcf,0x7a,0xeb,
  	};
  int n=4; 

/*Caja de SAND[7] S1 separada FBU=16*/
/*	unsigned char SBOX[] = {
  	    0x00,0x01,0x02,0x0b,0x04,0x05,0x06,0x0f,
  	    0x08,0x09,0x0a,0x03,0x0d,0x0c,0x07,0x0e,
  	};
  int n=4;*/
  /*Caja de SAND[7] S2 separada FBU=16*/
/*	unsigned char SBOX[] = {
  	    0x00,0x01,0x02,0x03,0x04,0x07,0x06,0x05,
  	    0x08,0x09,0x0e,0x0d,0x0c,0x0f,0x0a,0x0b,
  	};
  int n=4;*/

  /*Caja de Scenery[36] usa las mismas cajas que Rectangle FBU=8*/
	/*unsigned char SBOX[] = {
  	    0x06,0x05,0x0c,0x0a,0x01,0x0e,0x07,0x09,
  	    0x0b,0x00,0x03,0x0D,0x08,0x0f,0x04,0x02,
  	};
  int n=4;*/

  /*Caja de WARP[104]*/
	/*unsigned char SBOX[] = {
  	    0x0c,0x0a,0x0d,0x03,0x0e,0x0b,0x0f,0x07,
  	    0x08,0x09,0x01,0x05,0x00,0x02,0x04,0x06,
  	};
  int n=4;


    /*Cajas de LBlock S0[ ]
	unsigned char SBOX[] = {
  	    0x0e,0x09,0x0f,0x00,0x0d,0x04,0x0a,0x0b,
  	    0x01,0x02,0x08,0x03,0x07,0x06,0x0c,0x05,
  	};
  int n=4;*/

    /*Cajas de LBlock S1[ ]
	unsigned char SBOX[] = {
  	    0x04,0x0b,0x0e,0x09,0x0f,0x0d,0x00,0x0a,
  	    0x07,0x0c,0x05,0x06,0x02,0x08,0x01,0x03,
  	};
  int n=4;	*/

	/*Cajas de LBlock S2[ ]
	unsigned char SBOX[] = {
  	    0x01,0x0e,0x07,0x0c,0x0f,0x0d,0x00,0x06,
  	    0x0b,0x05,0x09,0x03,0x02,0x04,0x08,0x0a,
  	};
  int n=4;*/

	/*Cajas de LBlock S3[ ]
	unsigned char SBOX[] = {
  	    0x07,0x06,0x08,0x0b,0x00,0x0f,0x03,0x0e,
  	    0x09,0x0a,0x0c,0x0d,0x05,0x02,0x04,0x01,
  	};
  int n=4;*/

	/*Cajas de LBlock S4[ ]
	unsigned char SBOX[] = {
  	    0x0e,0x05,0x0f,0x00,0x07,0x02,0x0c,0x0d,
  	    0x01,0x08,0x04,0x09,0x0b,0x0a,0x06,0x03,
  	};
  int n=4;*/

	/*Cajas de LBlock S5[ ]
	unsigned char SBOX[] = {
  	    0x02,0x0d,0x0b,0x0c,0x0f,0x0e,0x00,0x09,
  	    0x07,0x0a,0x06,0x03,0x01,0x08,0x04,0x05,
  	};
  int n=4;*/

	/*Cajas de LBlock S6[ ]
	unsigned char SBOX[] = {
  	    0x0b,0x09,0x04,0x0e,0x00,0x0f,0x0a,0x0d,
  	    0x06,0x0c,0x05,0x07,0x03,0x08,0x01,0x02,
  	};
  int n=4;*/

	/*Cajas de LBlock S4[ ]
	unsigned char SBOX[] = {
  	    0x0d,0x0a,0x0f,0x00,0x0e,0x04,0x09,0x0b,
  	    0x02,0x01,0x08,0x03,0x07,0x05,0x0c,0x06,
  	};
  int n=4;
  
  printf("LBlock s7 \n");*/

     
    /*Cajas de TWINE[ ]*/
	/*unsigned char SBOX[] = {
  	    0x0c,0x00,0x0f,0x0a,0x02,0x0b,0x09,0x05,
  	    0x08,0x03,0x0d,0x07,0x01,0x0e,0x06,0x04,
  	};
  int n=4;



	//Caja de CLEFIA S0[ ]*/
	/*unsigned char SBOX[] = {
  	    ,
  	};
  int n=8;

	/*Caja de CLEFIA S1[ ]*/
	unsigned char SBOX[] = {
  	    ,
  	};
  int n=8;

  
  /***********************Sboxes_F************************/


	//FBCT(SBOX,n);
	//LAT(SBOX,n,0);
        //DDT(SBOX,n,0);
	FBDT(SBOX,n);
  printf("CLEFIA\n");
	return 0;
}
